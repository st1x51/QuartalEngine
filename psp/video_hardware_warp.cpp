...
void Sky_LoadSkyBox (char *name)
{
    ...
    skypal = static_cast<byte*>(COM_LoadHunkFile (skypalette));
    if (!skypal)
    {
#ifdef  SLIM_MODEL
        char    size[64] = "_";
    ...
    for (i=0 ; i<6 ; i++)
    {
        ...
        if (f) // Load up 512 x 512 sky textures if on slim psp.
        {
            if (!Q_strncmp(size,"_512_",5))
                skyimage[i] = GL_LoadPaletteTexture ("", 512, 512, f+8, (byte *)skypal, PAL_RGB, qfalse, GU_LINEAR, 0);
            else // If no 512 x 512 textures are available, use 256 x 256.
                skyimage[i] = GL_LoadPaletteTexture ("", 256, 256, f+8, (byte *)skypal, PAL_RGB, qfalse, GU_LINEAR, 0);
...
void Sky_NewMap (void)
{
	...
	while (1)
	{
        ...
        else if (!strcmp("", skybox_name)) // Solitude loads a sky by default if none is specified
            Sky_LoadSkyBox ("sky");
		else if (!strcmp("skyname", key)) //half-life
            Sky_LoadSkyBox(value);
        else if (!strcmp("qlsky", key)) //quake lives
            Sky_LoadSkyBox(value);
        else if (!strcmp("sky", key))
                Sky_LoadSkyBox(value);
	}
}
