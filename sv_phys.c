...
int SV_FlyMove (edict_t *ent, float time, trace_t *steptrace)
{
	...
	for (bumpcount=0 ; bumpcount<numbumps ; bumpcount++)
	{
		...
		// g-cont. collide with attached cube
		if( PROG_TO_EDICT( ent->v.aiment ) != EDICT_NUM( 0 ))
		{
			edict_t *aim = PROG_TO_EDICT( ent->v.aiment );

			old_cube_owner = aim->v.owner;
			aim->v.solid = SOLID_SLIDEBOX;// make turret or cube solid
			aim->v.owner = 0;		// reset owner so trace can grab him as normal
			SV_LinkEdict( aim, false );	// temporare move to solid list
		}
		...
		if( old_cube_owner != 0 )
		{
			...
			aim->v.owner = old_cube_owner;
			aim->v.solid = SOLID_NOT;	// SV_ClipToLinks will be ignore not solid entities so we
						// don't need to call SV_LinkEdict here
		}
